...
#                        -c <comma-separated list of colors for each population>
...
# Usage example: plotADMIXTURE.r -p fileXY -i file.ind.pop.txt -k 4 -l pop1,pop2,pop3 -c red,blue,green
...
# with red, blue, and green colors respectively.
...
# Script modified by me from original author: Joana Meier
...
option_list = list(
  ...
  make_option(c("-c", "--colors"), type="character", default=NULL, 
              help="comma-separated list of colors for each population", metavar="character"),
...
}else if (is.null(opt$colors)){
  print_help(opt_parser)
  stop("Please provide a comma-separated list of colors for each population", call.=FALSE)
}
...
tiff(file=paste0(opt$outPrefix,".tiff"),width = 2000, height = 1200,res=200)
 ...
 # Plot minK
 bp<-barplot(t(as.matrix(tbl[[1]][order(labels$n),])), col=unlist(strsplit(opt$colors, ",")),xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces)
 axis(3,at=bp,labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.6)
 # Plot higher K values
 if(maxK>minK){
   for(x in 2:(maxK-1)) {
     barplot(t(as.matrix(tbl[[x]][order(labels$n),])), col=unlist(strsplit(opt$colors, ",")),xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces)
   }
 }
